{ lib }:
{
  mkHomeConfigurations =
    ...
    let
      ...
      availableUsers =
        if builtins.pathExists homeDir then lib.attrNames (builtins.readDir homeDir) else [ ];

      # Create configs for each user that has a config for this host
      mkHomeConfigsForSystem =
        system:
        let
          pkgs = mkPkgs system;
          ...
          mkHomeConfig =
            ...
            let
              userConfigs =
                lib.genAttrs
                  (lib.filter (user: builtins.pathExists (homeDir + "/${user}/${host}.nix")) availableUsers)
                  (
                    user:
      ...
      allConfigs = lib.foldl (acc: system: acc // (mkHomeConfigsForSystem system)) { } systems;
    in
    allConfigs;
}
