...
function GpuAdapters:init()
  ...
  for _, adapter in ipairs(self.ENUMERATED_GPUS) do
    if not initial[adapter.device_type] then initial[adapter.device_type] = {} end
...
function GpuAdapters:pick_best()
  ...
  if not adapters_options then adapters_options = self.IntegratedGpu end

  if not adapters_options then adapters_options = self.Other end

  if not adapters_options then adapters_options = self.Cpu end
