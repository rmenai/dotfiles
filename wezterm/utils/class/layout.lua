...
function M:new(name)
  ...
  return setmetatable({ layout = {}, log = Logger:new(name), name = name }, { __index = self })
end
...
function M:insert(action, background, foreground, text, attributes)
  if not action or action == "" then return self.log:error("Cannot operate with empty action") end

  local idx = function() return action == "prepend" and 1 or #self + 1 end
  ...
  for i = 1, #colors_pair do
    ...
    local color_entry = ansi_colors[color] and { [attr] = { AnsiColor = color } } or { [attr] = { Color = color } }
  ...
  if attributes then
    for i = 1, #attributes do
      ...
      if not attribute_mappings[attribute] then return self.log:error("attribute '%s' is not defined!", attribute) end
...
function M:append(background, foreground, text, attributes) return M.insert(self, "append", background, foreground, text, attributes) end

---Prepend the given element in the Layout.
---@see Utils.Class.Layout.insert
function M:prepend(background, foreground, text, attributes) return M.insert(self, "prepend", foreground, background, text, attributes) end
...
function M:debug(formatted) self.log:info(formatted and self:format() or self) end
