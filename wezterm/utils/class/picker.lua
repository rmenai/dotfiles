...
function h.normalize(item) return type(item) == "table" and item or { id = item } end
...
function h.is_array(item) return type(item) == "table" and item[1] ~= nil end
...
function h.build(items, comp)
  ...
  table.sort(choices, comp or function(a, b) return a.id < b.id end)
...
function h.path_to_module(path) return (path:sub(#config_dir + 2):gsub("%.lua$", ""):gsub(fs.path_separator, ".")) end
...
function M:select(Overrides, opts)
  ...
  if not choice then return self.log:error("%s is not defined for %s", opts.id, self.title) end
