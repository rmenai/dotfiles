...
--~~ {{{2 string
local schar, sfind, sformat, sgsub, smatch, ssub, srep = string.char, string.find, string.format, string.gsub, string.match, string.sub, string.rep
--~~ }}}
...
M.g.memoize = function(key, value)
  ...
  if value then CACHE[key] = CACHE[key] or value end
  return CACHE[key]
end --~~ }}}
...
M.fs.home = M.g.memoize("home", function() return (sgsub((ogetenv("USERPROFILE") or ogetenv("HOME") or wt_home or ""), "\\", "/")) end) --~~ }}}
...
M.fs.basename = function(path)
  ...
  if CACHE["basename"][path] then return CACHE["basename"][path] end
...
M.fs.find_git_dir = function(directory)
  ...
  if CACHE["git-dir"][directory] then return CACHE["git-dir"][directory] end
  ...
  if cwd_uri then
    if type(cwd_uri) == "userdata" then
      ...
      if slash then
        ...
        cwd = sgsub(cwd_uri, "%%(%x%x)", function(hex) return schar(tonumber(hex, 16)) end)
      end
    end

    local dot = sfind(hostname, "[.]")
    if dot then hostname = ssub(hostname, 1, dot - 1) end
    if hostname == "" then hostname = wt_hostname() end
...
M.fs.pathconcat = function(...) return tconcat({ ... }, M.fs.path_separator) end --~~ }}}
...
M.fs.ls_dir = function(directory)
  ...
  if CACHE["read-dir"][directory] then return CACHE["read-dir"][directory] end
  ...
  if file then
    ...
    if not success then return M.fs.log:error("[ls_dir] Unable to create temp file!") end
...
---@return boolean
M.key.__has = function(lhs, pattern) return sfind(lhs, pattern) ~= nil end --~~ }}}
...
M.key.map = function(lhs, rhs, tbl)
  ...
  local function __map(key, mods) tbl[#tbl + 1] = { key = key, mods = tconcat(mods or {}, "|"), action = rhs } end

  ---initialize the modifiers table
  local mods = {}

  ---dont'parse a single key
  if #lhs == 1 then return __map(lhs, mods) end

  local aliases, modifiers = M.key.aliases, M.key.modifiers
  lhs = M.key.__has_leader(lhs, mods)

  if not M.key.__has(lhs, "%b<>") then return __map(lhs, mods) end

  lhs = sgsub(lhs, "(%b<>)", function(str) return ssub(str, 2, -2) end)

  local keys = M.str.split(lhs, "%-")

  if #keys == 1 then return __map(aliases[keys[1]] or keys[1], mods) end
...
M.mt.round = function(number) return mfloor(number + 0.5) end --~~ }}}
...
M.mt.toint = function(number, increment)
  if increment then return mfloor(number / increment) * increment end
...
M.str.trim = function(s) return (sgsub(s, "^%s*(.-)%s*$", "%1")) end --~~ }}}
...
M.str.split = function(s, sep, opts)
  ...
  if CACHE["split"][key] then return CACHE["split"][key] end
...
M.str.format_tab_title = function(pane, title, config, max_width)
  ...
  if Icon.Progs[process] then title = Icon.Progs[process] .. " " .. (other or "") end

  local proc = pane.foreground_process_name or pane:get_foreground_process_name() or ""
  if sfind(proc, "nvim") then proc = ssub(proc, sfind(proc, "nvim") or 1) end
  ...
  if proc == "nvim" then
    ...
    local full_cwd = pane.current_working_dir and pane.current_working_dir.file_path or pane:get_current_working_dir().file_path
    ...
    if max_width == config.tab_max_width then cwd = wt_truncate_rx(cwd, max_width - 14) .. "..." end
  ...
  if is_truncation_needed and max_width == config.tab_max_width then title = wt_truncate_rx(title, max_width - 8) .. "..." end
...
M.color.set_tab_button = function(Config, theme)
  ...
  for i = 1, #states do
    ...
    local attributes = {
      style.intensity or (style.italic and "Italic") or (style.strikethrough and "Strikethrough") or (style.underline ~= "None" and style.underline),
    }
