...
e.__find_best_fit = function(combinations, max_width, sep_width, pad_width)
  ...
  for i = 1, #combinations do
    local total_width = e.__compute_combination_width(combinations[i], sep_width, pad_width)
...
e.set_left_status = function(window)
  ...
  if ws ~= "" and not e.mode then
    ...
    if e.width.usable >= e.width.ws then lsb:append(ws_bg, e.bg, ws, { "Bold" }) end
  end
...
e.set_modal_prompts = function(window)
  ...
  local prompt_bg, map_fg, txt_fg = e.theme.tab_bar.background, e.modes[e.mode].bg, e.theme.foreground
  ...
  for idx = 1, #key_tbl do
    ...
    if map:find("%b<>") then map = map:gsub("(%b<>)", function(s) return s:sub(2, -2) end) end
    ...
    if e.width.usable > 0 and desc ~= "" then
      ...
      if idx < #key_tbl and next_prompt_len < e.width.usable then rsb:append(prompt_bg, e.theme.brights[1], str.padr(msep, 1), { "NoItalic" }) end
    end
  end
...
e.update_width = function(Config, window, pane)
  for _ = 1, #window:mux_window():tabs() do
    ...
    e.width.tabs = e.width.tabs + str.width(str.format_tab_title(pane, tab_title, Config, 25))
  end
...
e.set_right_status = function(Config, window, pane)
  ...
  local hostname_cells = { hostname .. hostname_ico, hostname:sub(1, 1) .. hostname_ico, hostname_ico }
  --~ }}}
  ...
  -- if battery then tinsert(sets, battery.cells) end

  local cells = tbl.reverse(e.__find_best_fit(tbl.cartesian(sets), e.width.usable, str.width(sep.sb.right), 5))
